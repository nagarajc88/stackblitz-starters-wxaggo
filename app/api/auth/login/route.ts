...
export async function POST(req: NextRequest) {
  try {
      ...
      const user = await prisma.user.find((user:any)=> user.email == data.email);
      ...
      await Promise.all([response.cookies.set(cookieOptions), response.cookies.set({ name: "logged-in", value: "true", maxAge: tokenMaxAge })]);
      return response;
  } catch (error: any) {
